...
function sortByKeyword(recipes) {
  ...
  for (let i = 0; i < recipes.length; i++) {
    if (
      recipes[i].name.toLowerCase().includes(searchTerm.toLowerCase()) ||
...
function sortByIngredients(recipes) {
  ...
  const selectedIngredients = selectedTags.map((tag) => tag.textContent);
  ...
  for (let i = 0; i < recipes.length; i++) {
    ...
    for (let j = 0; j < selectedIngredients.length; j++) {
      ...
      if (
        recipe.name.toLowerCase().includes(selectedIngredient) ||
        recipe.description.toLowerCase().includes(selectedIngredient)
      ) {
...
function sortByAppliance(recipes) {
  ...
  const selectedAppliance = selectedTag.textContent;
  const filteredRecipes = [];
